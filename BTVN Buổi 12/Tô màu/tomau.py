...
class MapColoringApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("HCMC Ward Coloring Tool - Multi-Algorithm System")
        ...
        self.contours = []  
        self.color_assignment = {} # Lưu màu đã gán cố định cho thuật toán 2

        # Phân tích cấu trúc ảnh để tìm các vùng phường khép kín ngay từ đầu
        self.detect_all_wards()

        # Tạo cấu trúc dữ liệu wards_data động
        self.wards_data = []
        ...
        # Bảng màu Palette màu sắc chuẩn
        self.color_palette = [
            (76, 204, 46), (219, 152, 52), (182, 89, 155), (15, 196, 241),
            (34, 126, 230), (60, 76, 231), (156, 188, 26), (18, 156, 243),
            (120, 40, 200), (200, 100, 50), (40, 180, 150)
        ]
        ...
        # MENU THẢ XUỐNG ĐỂ CHỌN THUẬT TOÁN (TÍNH NĂNG MỚI)
        ...
        self.algo_option = ctk.CTkOptionMenu(
            self.sidebar, 
            values=["Sequential Color", "Backtracking (No-Adjacent-Overlap)"],
        ...
        # DANH SÁCH CUỘN CHỨA CÁC PHƯỜNG
        ...
        self.btn_download = ctk.CTkButton(self.sidebar, text="Download Map (PNG)", fg_color="#e0f2fe", text_color="#0369a1", hover_color="#bae6fd", font=ctk.CTkFont(weight="bold"), corner_radius=6, command=self.reset_map)
    ...
    def detect_all_wards(self):
        ...
        for c in contours:
            ...
            if aspect_ratio > 8.0 and y > int(h_img * 0.8):
                continue
                
            if area < 150: 
                continue
                
            if area > (total_image_area * 0.65):
                continue
    ...
    # THUẬT TOÁN 2 (MỚI): BACKTRACKING / GREEDY XÁC ĐỊNH PHƯỜNG KỀ NHAU ĐỂ TRÁNH TRÙNG MÀU
    def compute_adjacency_and_colors(self):
        n = len(self.contours)
        adj_matrix = np.zeros((n, n), dtype=bool)
        
        # Phóng to nhẹ bounding box biên để tìm các cặp contour sát nhau (khoảng cách biên < 15 pixel)
        boxes = [cv2.boundingRect(c) for c in self.contours]
        for i in range(n):
            for j in range(i + 1, n):
                x1, y1, w1, h1 = boxes[i]
                x2, y2, w2, h2 = boxes[j]
                
                # Check khoảng cách xa để loại bớt phép tính phức tạp
                if abs(x1 - x2) < max(w1, w2) + 15 and abs(y1 - y2) < max(h1, h2) + 15:
                    # Kiểm tra khoảng cách ngắn nhất giữa hai biên đa giác
                    dist = cv2.matchShapes(self.contours[i], self.contours[j], cv2.CONTOURS_MATCH_I1, 0)
                    # Một cách đơn giản và nhanh: Tính tâm khối
                    M1 = cv2.moments(self.contours[i])
                    M2 = cv2.moments(self.contours[j])
                    if M1["m00"] != 0 and M2["m00"] != 0:
                        cx1, cy1 = int(M1["m10"]/M1["m00"]), int(M1["m01"]/M1["m00"])
                        cx2, cy2 = int(M2["m10"]/M2["m00"]), int(M2["m01"]/M2["m00"])
                        center_dist = np.sqrt((cx1 - cx2)**2 + (cy1 - cy2)**2)
                        if center_dist < (max(w1, h1) + max(w2, h2)) * 0.65:
                            adj_matrix[i][j] = True
                            adj_matrix[j][i] = True

        # Thuật toán tô màu đồ thị Backtracking phối hợp Greedy không trùng màu kề
        result_colors = {}
        for i in range(n):
            available_palette_indices = [True] * len(self.color_palette)
            for neighbor in range(n):
                if adj_matrix[i][neighbor] and neighbor in result_colors:
                    assigned_color_idx = result_colors[neighbor]
                    if assigned_color_idx < len(available_palette_indices):
                        available_palette_indices[assigned_color_idx] = False
            
            # Chọn màu hợp lệ đầu tiên còn trống
            chosen_idx = 0
            for idx, allowed in enumerate(available_palette_indices):
                if allowed:
                    chosen_idx = idx
                    break
            result_colors[i] = chosen_idx
    ...
    def start_auto_coloring(self):
        ...
        if selected_algo == "Backtracking (No-Adjacent-Overlap)":
            # Tính toán phân phối màu không trùng kề trước khi chạy bước động họa
            self.color_assignment = self.compute_adjacency_and_colors()
        else:
            self.color_assignment = {i: (i % len(self.color_palette)) for i in range(len(self.contours))}
        
        self.animate_contour_step(0)
    ...
    def animate_contour_step(self, current_idx):
        ...
        palette_idx = self.color_assignment.get(current_idx, current_idx % len(self.color_palette))
        ...
        self.after(400, lambda: self.animate_contour_step(current_idx + 1))
