...
class VacuumCleanerAI:
    ...
    def build_ui(self):
        ...
        self.container = tk.Frame(self.algorithm_drawer, bg="#131a30")
        self.container.pack(fill="both", expand=True, side="top")
        self.canvas_scroll = tk.Canvas(self.container, bg="#131a30", highlightthickness=0)
        self.scrollbar = tk.Scrollbar(self.container, orient="vertical", command=self.canvas_scroll.yview)
        self.buttons_container = tk.Frame(self.canvas_scroll, bg="#131a30")
        self.buttons_container.bind(
            "<Configure>",
            lambda e: self.canvas_scroll.configure(scrollregion=self.canvas_scroll.bbox("all"))
        )
        self.canvas_scroll.create_window((0, 0), window=self.buttons_container, anchor="nw", width=300)
        ...
        self.create_drawer_button("RANDOM RESTART HILL CLIMBING", self.run_random_restart_hill_climbing)
        self.create_drawer_button("SIMULATED ANNEALING", self.run_simulated_annealing)
        self.create_drawer_button("LOCAL BEAM SEARCH", self.run_local_beam_search)
        self.create_drawer_button("GENETIC ALGORITHM (GA)", self.run_genetic)
        self.create_drawer_button("PARTICLE SWARM (PSO)", self.run_pso)
        self.create_drawer_button("ANT COLONY (ACO)", self.run_aco)
        self.create_drawer_button("AND-OR GRAPH SEARCH", self.run_and_or_graph_search)
    ...
    def stochastic_hill_climbing_algorithm(self):
        ...
        while True:
            ...
            if not better_neighbors:
                ...
                current = next_state
        return full_path
    def random_restart_hill_climbing_algorithm(self):
        best_path = []
        for _ in range(10):
            temp_room = copy.deepcopy(self.room)
            current = self.robot_position
            current_path = [current]
            def room_value(room_state):
                return -sum(row.count(1) for row in room_state)
            while True:
                if temp_room[current[0]][current[1]] == 1:
                    temp_room[current[0]][current[1]] = 0
                if sum(row.count(1) for row in temp_room) == 0:
                    break
                neighbors = self.get_neighbors(*current, temp_room)
                better_neighbors = []
                for neighbor in neighbors:
                    ...
                    if next_room[neighbor[0]][neighbor[1]] == 1:
                        next_room[neighbor[0]][neighbor[1]] = 0
                    if room_value(next_room) > room_value(temp_room):
                        better_neighbors.append(neighbor)
                if better_neighbors:
                    current = random.choice(better_neighbors)
                    current_path.append(current)
                else:
                    dirt_cells = [(r, c) for r in range(self.rows) for c in range(self.cols) if temp_room[r][c] == 1]
                    if not dirt_cells: break
                    nearest_dirt = min(dirt_cells, key=lambda pos: self.Manhattan_distance(current, pos))
                    frontier = deque([(current, [current])])
                    reached = {current}
                    ...
                    while frontier:
                        ...
                        if node == nearest_dirt:
                            found_path = path
                            break
                        for n in self.get_neighbors(*node, temp_room):
                            if n not in reached:
                                reached.add(n)
                                frontier.append((n, path + [n]))
                    if found_path and len(found_path) > 1:
                        current_path.extend(found_path[1:])
    ...
    def simulated_annealing_algorithm(self):
        ...
        while True:
            ...
            if random.random() < (T / 100.0):
                ...
                target = min(dirt_cells, key=lambda pos: self.Manhattan_distance(current, pos))
                next_state = min(neighbors, key=lambda n: self.Manhattan_distance(n, target))
            current = next_state
            full_path.append(current)
            T = max(T * alpha, Tmin)
        return full_path
    def local_beam_search_algorithm(self, k=3):
        start = self.robot_position
        full_path = [start]
        visited = {start} 
        temp_room = copy.deepcopy(self.room)
        current = start
        while sum(row.count(1) for row in temp_room) > 0:
            neighbors = self.get_neighbors(*current, temp_room)
            ...
            candidates.sort(key=lambda s: (temp_room[s[0]][s[1]] == 0, 
                                          min([abs(s[0]-r)+abs(s[1]-c) for r in range(self.rows) for c in range(self.cols) if temp_room[r][c]==1] or [0])))
    ...
    def genetic_algorithm(self):
        """Thuật toán Di truyền (GA) - Nhóm 4"""
        temp_room = copy.deepcopy(self.room)
        start = self.robot_position
        full_path = [start]
        dirt_cells = [(r, c) for r in range(self.rows) for c in range(self.cols) if temp_room[r][c] == 1]
        if not dirt_cells:
            return full_path
        def get_path_length(sequence):
            dist = self.Manhattan_distance(start, sequence[0])
            for idx in range(len(sequence) - 1):
                dist += self.Manhattan_distance(sequence[idx], sequence[idx+1])
            return dist
        population_size = 20
        generations = 30
        population = []
        for _ in range(population_size):
            individual = list(dirt_cells)
            random.shuffle(individual)
            population.append(individual)
        for _ in range(generations):
            population.sort(key=get_path_length)
            survivors = population[:population_size // 2]
            new_population = list(survivors)
            while len(new_population) < population_size:
                p1, p2 = random.sample(survivors, 2)
                cut = random.randint(1, len(dirt_cells) - 1)
        ...
        current_pos = start
        for next_dirt in best_order:
            frontier = deque([(current_pos, [current_pos])])
            reached = {current_pos}
            sub_path = None
            while frontier:
                node, path = frontier.popleft()
    ...
    def pso_algorithm(self):
        """Thuật toán Tối ưu hóa bầy đàn (PSO) - Nhóm 4"""
        temp_room = copy.deepcopy(self.room)
        start = self.robot_position
        full_path = [start]
        current = start
        while True:
            ...
            if not neighbors:
                break
            g_best = min(dirt_cells, key=lambda pos: self.Manhattan_distance(current, pos))
            def evaluate_particle(node):
                return self.Manhattan_distance(node, g_best) + random.uniform(0, 1)
            best_neighbor = min(neighbors, key=evaluate_particle)
            current = best_neighbor
            full_path.append(current)
    ...
    def aco_algorithm(self):
        """Thuật toán Thuộc địa kiến (ACO) - Nhóm 4"""
        temp_room = copy.deepcopy(self.room)
        start = self.robot_position
        full_path = [start]
        current = start
        ...
        while True:
            ...
            if not neighbors:
                break
            def get_ant_choice_score(node):
                nearest_dirt = min(dirt_cells, key=lambda pos: self.Manhattan_distance(node, pos))
                distance = self.Manhattan_distance(node, nearest_dirt)
                visibility = 1.0 / (distance + 0.5)
                return (pheromone[node] ** 1.0) * (visibility ** 2.0)
            best_neighbor = max(neighbors, key=get_ant_choice_score)
            ...
            if len(full_path) > 120:
                break
        return full_path
    def and_or_graph_search_algorithm(self):
        start = self.robot_position
        full_path = [start]
        temp_room = copy.deepcopy(self.room)
        current = start
        while True:
            if temp_room[current[0]][current[1]] == 1:
                temp_room[current[0]][current[1]] = 0
            dirt_left = sum(row.count(1) for row in temp_room)
            if dirt_left == 0:
                break
            def OR_SEARCH(state, path_list):
                if temp_room[state[0]][state[1]] == 1:
                    return []
                if state in path_list:
                    return None
                for neighbor in self.get_neighbors(*state, temp_room):
                    result_states = [neighbor] 
                    plan = AND_SEARCH(result_states, path_list + [state])
                    if plan is not None:
                        return [neighbor] + plan
                return None
            def AND_SEARCH(states, path_list):
                combined_plan = []
                for s in states:
                    plan_s = OR_SEARCH(s, path_list)
                    if plan_s is None:
                        return None 
                    combined_plan.extend(plan_s)
                return combined_plan
            target_path = OR_SEARCH(current, [])
            if target_path is None or len(target_path) == 0:
                break
            full_path.extend(target_path)
            current = target_path[-1]
            temp_room[current[0]][current[1]] = 0
        return full_path
    ...
    def run_random_restart_hill_climbing(self): self.reset_animation(); self.animate(self.random_restart_hill_climbing_algorithm(), "RANDOM RESTART HILL CLIMBING")
    def run_simulated_annealing(self): self.reset_animation(); self.animate(self.simulated_annealing_algorithm(), "SIMULATED ANNEALING")
    def run_local_beam_search(self): self.reset_animation(); self.animate(self.local_beam_search_algorithm(k=3), "LOCAL BEAM SEARCH")
    def run_genetic(self): self.reset_animation(); self.animate(self.genetic_algorithm(), "GENETIC ALGORITHM (GA)")
    def run_pso(self): self.reset_animation(); self.animate(self.pso_algorithm(), "PARTICLE SWARM OPTIMIZATION (PSO)")
    def run_aco(self): self.reset_animation(); self.animate(self.aco_algorithm(), "ANT COLONY OPTIMIZATION (ACO)")
    def run_and_or_graph_search(self): self.reset_animation(); self.animate(self.and_or_graph_search_algorithm(), "AND-OR GRAPH SEARCH")
